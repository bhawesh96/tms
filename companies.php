					...
					if(mysql_num_rows($result) > 0) {
					    try{
						    while($row = mysql_fetch_assoc($result)) {
						        ...
						        $modal_id = $company_name.(string)i;

						        $company_id = $row['id'];



						        $sql2 = "SELECT * FROM company_modals WHERE id = '$company_id'";
					  ...
					  <div class='modal-dialog'>
					    <div class='modal-content'>
					      <div class='modal-header'>
					        ...
					        <h4 class='modal-title'>$modal_title for $subject</h4>
					      </div>
								...
								<div class='panel panel-default'>
								    <div class='panel-heading'>
								      <h4 class='panel-title'>
								        <a data-toggle='collapse' data-parent='#accordion' href='#$course_id'>
								        $course_name</a>
								      </h4>
								    </div>
								    <div id='$course_id' class='panel-collapse collapse'>
								      <div class='panel-body'>$course_description<br>
								      <a href='$pdf_link' download>Download detailed PF</a>
								      </div>
								    </div>
								  </div>";
...
";
						        ...
						        $accordion_id1 = $company_name.(string)(i+1);
						        $accordion_id2 = $company_name.(string)(i+2);
						        $accordion_id3 = $company_name.(string)(i+3);

					//$sql2 = "SELECT * FROM company_modals WHERE id=
						  print "
						  	<div class='col-md-4 member-team' id='$company_name'>
							<a data-toggle='modal' data-target='#$modal_id'>
								<img src='$image_link' alt='' class='img-responsive'>
								<h4>$company_name</h4>
								<p>$description</p>
							</div></a>
		...
		<script>
		$('#<?php echo $company_name ?>').click(function(){
